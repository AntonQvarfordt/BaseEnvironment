...
#define CUSTOM_SAMPLE_SHADOW_PROJ(tex,coord) ((SAMPLE_DEPTH_TEXTURE_PROJ(tex,UNITY_PROJ_COORD(coord)) < ((coord).z/(coord).w)) ? 0.0 : 1.0)
	...
	float2 IntersectionPoints(float3 rayDir)
	{
		...
		bottom.y = dot(_CustomLightPosition - _WorldSpaceCameraPos, BottomFrustumNormal);

		bottomDenom.z = dot(LeftFrustumNormal, rayDir);
		bottom.z = dot(_CustomLightPosition - _WorldSpaceCameraPos, LeftFrustumNormal);
		...
		top.y = dot(_CustomLightPosition - _WorldSpaceCameraPos, TopFrustumNormal);

		topDenom.z = dot(RightFrustumNormal, rayDir);
		top.z = dot(_CustomLightPosition - _WorldSpaceCameraPos, RightFrustumNormal);
...
float SampleShadows(float2 suv,float zdis)
{
#ifndef SHADOWS_OFF

	
#if defined(UNITY_REVERSED_Z)
float z_buffer_value = 1 - (1.0 - (zdis * _LightParams.z) * (ShadowBias.y)) / ((zdis * _LightParams.z) * (ShadowBias.x));
#else
float z_buffer_value = (1.0 - (zdis * _LightParams.z) * (ShadowBias.y)) / ((zdis * _LightParams.z) * (ShadowBias.x));
#endif
return CUSTOM_SAMPLE_SHADOW(_ShadowMapTexture, float4(suv.xy, z_buffer_value, 0));
#else
return 1;
#endif
}
...
//#endif
	...
	for (int i = 0; i < NUM_SAMPLES; i++)
	{
...
#endif
		float cd = SampleShadows(suv, EnterPoint.z);
		...
		//add to slices....
		ThisMarch = lerp(LightColour2.rgb, LightColour.rgb, saturate(TintPercent * (min(1 - att, 1 - length(suv.xy - float2(0.5,0.5)) * 2)) / 2)) * max(((cd * phase) + (ShadowBias.z * (1 - cd)))* atten * exp(-extinction)* fogDensity * WolrdstepSize, 0);
